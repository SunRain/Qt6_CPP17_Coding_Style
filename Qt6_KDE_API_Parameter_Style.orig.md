...
- Public API 禁止持有 view/指针/引用的生命周期（成员、缓存、队列、lambda 捕获后延迟使用等都算持有）。
...
| 非 Qt 容器切片 | `std::span<const T> items` | 仅用于非常稳定且简单的切片入参；避免在公共头文件里引入大规模模板体系。 |
...
public:
    void setName(QAnyStringView name);        // 纯 C++ 入口：borrow/sink 入口
...
public:
    ...
    void setName(QAnyStringView name);  // convenience
private:
...
## 3) Internal：实现层允许用 sink（按值 + move）以保证简洁/性能

对外（public）可以保持“`const QString&` + view”的接口风格；对内（internal）推荐统一到一个 sink 实现。
...
void Foo::setName(QAnyStringView name)   { setNameImpl(d, name.toString()); }
```
