...
| Functions / local variables / parameters | camelCase | `void updateData()` | `void updatedata()` |
| Private/protected non-static state members of ordinary classes | `m_` + lowerCamelCase | `int m_count` | `int count` |
| Explicitly approved public non-static direct fields | lowerCamelCase, no prefix | `QUrl sourceUrl` | `QUrl m_sourceUrl` |
...
### 2.1 Direct-Field Data Types and Qt-Upstream Private Data Style

Member access and member naming must be decided in two stages. A naming rule must never be used to justify exposing state.

#### Stage 1: Approve the Direct-Field Model

Only the following two categories may be explicitly approved for public non-static state exposed as direct fields:

1. **Record-like data types**: the primary responsibility is carrying a set of data, direct field access is intentionally part of the type interface, and critical invariants do not depend on intercepting writes through setters. A small number of constructors, comparisons, or conversions does not change a type's record-like nature.
2. **Internal PIMPL / Qt shared-data implementation types**: the type is defined in a `.cpp`, `private/`, `_p.h`, or equivalent internal implementation file, is not exported, and is not part of the stable Public API. It either forms an explicit `Foo` / `FooPrivate` implementation relationship or is the internal data type held by `QSharedDataPointer<T>` / `QExplicitlySharedDataPointer<T>`.

None of the following facts grants approval on its own: using `class` or `struct`, already placing members under `public:`, using a `Private` / `Data` suffix, placing the type in an internal directory, or inheriting `QSharedData`. Ordinary managers, controllers, services, workers, and other behavioral classes remain encapsulated by default. Widening access to bypass the `m_` rule is forbidden.

For a record-like type in an already published Public API, public field names and types are source contracts, and field layout may also be an ABI contract. Approving direct fields means accepting those compatibility responsibilities.

#### Stage 2: Name Members Under the Approved Access Model

- Approved public non-static direct fields use unprefixed lowerCamelCase.
- Private/protected non-static state uses `m_` + lowerCamelCase, including non-public state in an otherwise approved type.
- Static state continues to use `s_`; this section does not redefine the existing static/global naming rule.
- `d`, `d_ptr`, `q_ptr`, and the local `d` / `q` variables introduced by `Q_D` / `Q_Q` are fixed Qt names and are not rewritten with ordinary member prefixes.

#### Record-Like Data Type
...
An ordinary behavioral class is not approved merely because a `struct` or public fields would be convenient:
...
#### Qt Shared-Data Implementation Types

`QSharedDataPointer<T>` provides implicit copy-on-write detaching: a write through a non-const access path detaches when needed. `QExplicitlySharedDataPointer<T>` does not detach automatically; when value-semantics copy-on-write is required, the holder calls `detach()` before writing. Public state in both data types follows the same naming rule; only the holder's write strategy differs.

In the following example, each `new T` is transferred immediately to its shared-data holder in the same constructor expression; no owning raw pointer remains.
...
public:
    ...
    {
        d->title = title; // Non-const access detaches as needed.
    }
...
A data object that can genuinely be shared by multiple public value objects must not, by default, store a per-owner `q_ptr` or use `Q_DECLARE_PUBLIC`. The holder owns the detach policy; that policy does not change data-field naming.
...
Member-name checks must not use an unconditional `MemberPrefix: 'm_'`. Configure access-specific rules and whitelist the fixed Qt d-pointer names:
...
clang-tidy checks lexical naming only. `PublicMemberPrefix: ''` does not authorize any type to expose state. A project must first use an AST allowlist or human review to confirm that a type is an approved record-like data type or internal PIMPL/shared-data implementation type, and must reject public state in ordinary behavioral classes. Type suffixes and `QSharedData` inheritance must never be automatic approval conditions.
...
- [ ] Private/protected non-static state uses `m_xxx`, including non-public state in approved types
- [ ] Static members use `s_xxx`; `q_ptr`/`d_ptr`/`d`/`q` keep their fixed Qt names
- [ ] Ordinary classes do not widen access to bypass the `m_` rule
- [ ] `QSharedDataPointer` writes rely on automatic detach; copy-on-write with `QExplicitlySharedDataPointer` explicitly detaches in the holder before writing
