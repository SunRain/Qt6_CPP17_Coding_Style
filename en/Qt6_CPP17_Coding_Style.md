...
| Private/protected non-static members of ordinary classes | `m_` + camelCase | `int m_count` | `int count` |
| Public state members of an internal PIMPL `FooPrivate` | camelCase, no prefix | `QUrl documentUrl` | `QUrl m_documentUrl` |
| Public fields of data-only structs | camelCase, no prefix | `int requestCount` | `int m_requestCount` |
...
### 2.1 Qt-Upstream-Style PIMPL Private Classes

Public state members may omit the `m_` prefix only when all of the following conditions hold:

1. The type forms an explicit `Foo` / `FooPrivate` pair with a public class.
2. The type is defined in a `.cpp`, `private/`, `_p.h`, or equivalent internal implementation file.
3. The type is not exported and is not part of the stable Public API.
4. The type implements `Foo` through `Q_DECLARE_PUBLIC(Foo)`, inheritance from `QObjectPrivate`, or an equivalent PIMPL relationship.
5. The no-prefix rule applies only to non-static state members in the private class's `public:` section.

Private/protected members of `FooPrivate` still use `m_`. Moving ordinary class members to `public:` to bypass the `m_` rule is forbidden.

Static members of PIMPL private classes continue to use `s_`. `q_ptr`, `d_ptr`, and the local `d` / `q` variables generated by `Q_D` / `Q_Q` are fixed Qt names and are excluded from ordinary member-name checks.

Generic Qt 6 example:
...
clang-tidy cannot fully determine whether a public field belongs to an approved PIMPL, a data-only struct, or an ordinary Public API class. Projects should add an AST or equivalent gate that recognizes the first two exceptions and rejects public state in ordinary Public API classes as a way to bypass `m_`.
...
- [ ] Private/protected members of ordinary classes use `m_xxx`
- [ ] Public state members of approved `FooPrivate` classes use unprefixed camelCase
- [ ] Public fields of data-only structs use unprefixed camelCase
